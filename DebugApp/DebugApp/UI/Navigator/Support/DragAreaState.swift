...
final class DragAreaState: ObservableObject {
    ...
    func move(start: CGPoint, current: CGPoint) {
        if !self.isDrag {
            ...
            self.isPointDrag = self.dragArea?.onStart(start: worldStart, radius: radius) ?? false
            self.isDrag = true
        }
        ...
        if self.isPointDrag {
            self.dragArea?.onMove(delta: CGSize(width: dx, height: dy))
        } else {
    ...
    func apply(start: CGPoint, current: CGPoint) {
        ...
        let dy = (current.y - start.y) * zoom
        if self.isPointDrag {
            self.dragArea?.onEnd(delta: CGSize(width: dx, height: dy))
    ...
    func reset() {
        self.zoom = DragAreaState.defaultZoom
        self.zoomOrigin = DragAreaState.defaultZoom
        self.viewPoint = .zero
